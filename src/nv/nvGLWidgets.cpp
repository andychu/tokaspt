...
//tbp: #include <nvglutils/nvShaderUtils.h>
...
//tbp: font tweaking
#if 1
	static void *tbp_font = GLUT_BITMAP_HELVETICA_12;
	static int tbp_font_height = 12, tbp_font_descent = 4;
#elif 0
	static void *tbp_font = GLUT_BITMAP_HELVETICA_10;
	static int tbp_font_height = 10, tbp_font_descent = 4;
#else
	static void *tbp_font = GLUT_BITMAP_8_BY_13;
	static int tbp_font_height = 12, tbp_font_descent = 4;
#endif
...
int GLUIPainter::getTextSize(const char * text, int& nbLines) const
{
    ...
    while(*text != '\0')
    {
        if (*text != '\n')
        {
            wLine += glutBitmapWidth( tbp_font, *text);
        }
        else
        {
            nbLines++;
...
void GLUIPainter::init()
{
    ...
    if (m_textListBase == 0)
    {
        ...
        for (int ii = 0; ii < 128; ii++) {
            ...
            glutBitmapCharacter( tbp_font, ii);
            glEndList();
        }
    }
}
