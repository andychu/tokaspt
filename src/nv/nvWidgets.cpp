...
void UIContext::end()
{
    m_painter->end();
	//tbp:
	// printf("nvui: index %3d       \r", m_groupIndex);
	assert(m_groupIndex == 0 && "unbalanced grouping");
...
void UIContext::beginGroup(int flags, const Rect& r)
{
	...
	//tbp: need to preserve that bit, but it doesn't need to propagate.
	int skip_frame = flags & GroupFlags_InternalSkipFrame;
	...
	//tbp: preservation part II
	newGroup.flags |= skip_frame;
...
{
    endGroup();
	//tbp: needed to skip collapsed panel frame.
	if (!(m_groupStack[m_groupIndex + 1].flags & GroupFlags_InternalSkipFrame)) 
    ...
    beginGroup( ( flags | GroupFlags_LayoutNoMargin |GroupFlags_LayoutNoSpace ) & GroupFlags_StartXMask , rpanel );
		...
		*/
		if (isUnfold && *isUnfold) {
			...
			beginFrame(flags | GroupFlags_InternalSkipFrame, Rect(0, 0, 0, 0)); // in fact would need a no-op flag.
			return false;
		}
                ...
                else if ( m_keyBuffer[keyNb] == '\b' )
                {
                    if (m_focusCaretPos > 0)
                    {
                        if (m_focusCaretPos != textLength)
                        {
							//tbp: 
							//BUG: blindfix.
                            // memmove( text + m_focusCaretPos - 1, text + m_focusCaretPos, textLength - m_focusCaretPos);							memmove( text + m_focusCaretPos - 1, text + m_focusCaretPos, max(0, textLength - m_focusCaretPos));
							memmove( text + m_focusCaretPos - 1, text + m_focusCaretPos, max(0, textLength - m_focusCaretPos));
                        }
                        m_focusCaretPos--;
                        textLength--; 
                    }
